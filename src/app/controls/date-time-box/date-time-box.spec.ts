...
describe('ErpDateTimeBox', () => {
  ...
  it('does not commit dismissal and commits one combined confirmation', async () => { const fixture = create(); const manager = TestBed.inject(ErpOverlayManager); const onChange = vi.fn(); fixture.componentInstance.registerOnChange(onChange); const trigger = (fixture.nativeElement as HTMLElement).querySelector('button') as HTMLButtonElement; trigger.click(); manager.entries()[0].ref.dismiss('cancel'); await Promise.resolve(); expect(onChange).not.toHaveBeenCalled(); trigger.click(); manager.entries()[0].ref.close('2026-05-04T09:35'); await Promise.resolve(); expect(onChange).toHaveBeenCalledOnce(); });
});
