...
describe('ErpSelectionPickerContent', () => {
  ...
  it('renders semantic swatches, a native custom color input, and commits canonical custom color', async () => { const {fixture, manager, ref} = open({...base, mode: 'color'}); const root = fixture.nativeElement as HTMLElement; expect(root.querySelectorAll('[data-color-preset]').length).toBe(7); const input = root.querySelector('[data-native-color]') as HTMLInputElement; input.value = '#abcdef'; input.dispatchEvent(new Event('input')); root.querySelector<HTMLButtonElement>('[data-confirm-action] button')?.click(); manager.completeTransition(ref.id, 'leaving'); await expect(ref.afterClosed).resolves.toEqual({type: 'closed', result: '#ABCDEF'}); });
  it('shows every semantic icon and supports keyboard selection', async () => { const {fixture, manager, ref} = open({...base, mode: 'icon', searchable: true}); const root = fixture.nativeElement as HTMLElement; expect(root.querySelectorAll('[data-icon-option]').length).toBe(ERP_ICON_NAMES.length); const list = root.querySelector('[data-selection-list]') as HTMLElement; list.dispatchEvent(new KeyboardEvent('keydown', {key: 'ArrowDown'})); list.dispatchEvent(new KeyboardEvent('keydown', {key: 'Enter'})); root.querySelector<HTMLButtonElement>('[data-confirm-action] button')?.click(); manager.completeTransition(ref.id, 'leaving'); await expect(ref.afterClosed).resolves.toEqual({type: 'closed', result: ERP_ICON_NAMES[1]}); });
