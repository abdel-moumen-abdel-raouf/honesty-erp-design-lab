...
describe('ErpDateRangeBox', () => {
  ...
  it('commits an ordered range only after confirm', async () => { const fixture = create(); const manager = TestBed.inject(ErpOverlayManager); const onChange = vi.fn(); fixture.componentInstance.registerOnChange(onChange); (fixture.nativeElement as HTMLElement).querySelector('button')?.click(); manager.entries()[0].ref.close({start: '2026-05-01', end: '2026-05-10'}); await Promise.resolve(); expect(onChange).toHaveBeenCalledWith({start: '2026-05-01', end: '2026-05-10'}); });
});
